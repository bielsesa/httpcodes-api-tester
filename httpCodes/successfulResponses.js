...
router.route('/').all((req, res) => {
    res.send('This will return Successful responses HTTP codes, that range from 200 to 299.');
});

router.route('/:httpCode').all((req, res) => {
    console.debug(req.params.httpCode);
    let httpCodeDefinition = httpCodesDefinition['successful-responses'][req.params.httpCode];
